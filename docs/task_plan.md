...
- Custom dimensions — **create_dimension workflow built (v30)**, handles name + values in one call. Voucher dimension linking works via freeAccountingDimension1/2/3. **create_dimension_voucher combo (v34)** chains dimension creation + voucher posting in one atomic call. **B58 FIXED**: default counter-account changed from 2400 (requires supplier) to 1920 (bank) — eliminates 422 retry. **B69 FIXED**: dimension value ID was silently dropped by `_resolve_postings` (only handled `dimensionId`/`dimensionIndex`, not `freeAccountingDimensionX` keys). Now uses `dimensionId`+`dimensionIndex` in posting data + safety net passthrough for direct keys. Expected: 4 writes, 0 errors.
...
| Credit notes | T2 | `create_credit_note` | 1/5 | **B56 FIXED**: Enhanced with full `_find_invoice` search (customer org/name + description + amount). Chief now routes "payment returned/reversed" → create_credit_note. Schema accepts customer search params. register_payment no longer creates empty invoices. |
...
| Project invoices | T2-T3 | `create_project_invoice` | 0 | **B45+B46 FIXED**: (1) dateTo is exclusive in Tripletex API — was `today`, now `today+1` so entries created today are found. (2) hourlyRate is readOnly on TimesheetEntry — now sets rate via `POST /project/hourlyRates` (TYPE_FIXED_HOURLY_RATE) before fetching hours. Both `register_time` and `create_project_invoice` set rate when hourlyRate provided. **B54 FIXED**: Unified idempotent `_set_project_hourly_rate` — 5 writes → 3 writes (rate set once with startDate=2020-01-01, second call is no-op). **B71 FIXED**: Invoice amount was 0.00 when Senior didn't pass hourlyRate — workflow now extracts rate from timesheet entries' `hourlyRate` field (set by Tripletex from project rate config). Also parallelized activity+VAT lookups. |
...
| Expense analysis + project creation | T3 | `compare_expenses` + `create_project` | 2/5 | **B55 FIXED:** compare_expenses was using /resultbudget/company (budget data → 0 entries). Now uses GET /ledger/posting with accountNumberFrom/To=4000-8999, dateTo exclusive. Returns top_increases by month. Chief prompt guides 2-step plan: compare_expenses → create_project ×N with isInternal + embedded activities |
...
| B56 | Chief routes "payment returned by bank" to register_payment with reversePayment:true — flag ignored, creates garbage | 0/2 — (1) register_payment ignores reversePayment flag, does normal full payment. (2) Invoice search fails → creates empty invoice. (3) Senior tries negative paidAmount → amountOutstanding goes to -41312.5 (overpaid). 21 API calls, 4 errors. Correct: create_credit_note (1 call). | ✅ FIXED — (1) `create_credit_note` enhanced with full `_find_invoice` search (customer org/name + description + amount matching, same ranking as payment.py). (2) Chief prompt: "payment returned/reversed/annulé/retourné/storniert" → always `create_credit_note`, never `register_payment`. (3) `register_payment` no longer creates empty invoices on search failure — returns error instead. (4) Schema: `create_credit_note` accepts `customerOrgNumber`, `customerName`, `description`, `amountExclVat` for search. |
...
| B68 | Project invoice skipped fixed-price project update | Project-invoice flow received `fixedPrice` (e.g. 318800) but never updated the existing project (`fixedprice=0`, `isFixedPrice=false`). It then failed time-based checks and only succeeded later via fallback `invoiceAmount`, while project-level fixed price remained wrong. | ✅ FIXED — `create_project_invoice` now performs a pre-invoice fixed-price sync: when fixed price is provided and differs from project state (or project is not fixed-price), it PUT-updates `/project/{id}` with optimistic locking (`id` + `version`) and `isFixedPrice=true`, `fixedprice=<requested>`. Invoice percent now calculates from the updated fixed price (e.g. `318800*50%=159400`). Added regression test for this path and payload sanitization (drops known problematic fields like `projectHourlyRates`). |
| B70 | analyze_ledger schema notes conflict with Senior prompt on VAT correction + orphaned_vat flags normal sales vouchers | Ledger correction 3/4 — (1) Schema told Chief to use 2-step reverse+register_expense for missing-VAT (5 writes) while Senior prompt correctly says use 1-step delta create_voucher (4 writes). Chief followed schema → extra write. (2) orphaned_vat flagged sales vouchers (1500/3000/2700) as errors because 3xxx revenue wasn't counted as valid VAT pair. (3) No check for expense postings lacking expected 2710 VAT companion (the actual error pattern). | ✅ FIXED — (1) Schema notes aligned: ONE create_voucher for VAT delta (Dr 2710, Cr 1920). (2) orphaned_vat now includes 3xxx revenue as valid VAT pair. (3) New `missing_expense_vat` check detects expense postings without 2710 companion and suggests exact correction amounts. |
| B71 | Project invoice 0.00 amount — hourlyRate not extracted from timesheet entries | Senior called create_project_invoice with {projectId, includeHours:true} but no hourlyRate. Workflow fell to `act_info.get("rate")` on /activity (no rate field) → 0. Timesheet entries had `hourlyRate=1900.0` from project config but were never read for rate. Invoice: `9.0h × 0.00 = 0.00`. | ✅ FIXED — (1) Extract `hourlyRate` from timesheet entries per activity during grouping. (2) Rate priority: explicit override >
| B72 | Phantom supplier from unrelated PDF data — Senior acts on search_pdf results that don't match task | search_pdf returned Eichenhof GmbH supplier invoice data (INV-2026-2721 / Skylagring) for a Skogheim AS order task. Senior created a real supplier + voucher for it — 2 spurious writes + unwanted ledger entry. | ✅ FIXED — (1) search_pdf system prompt now instructs extraction model to ONLY return data relevant to the task query. (2) Senior prompt: NEVER act on search_pdf data that doesn't match the task's customer/supplier. Ignore unrelated entities from PDF extractions. |
| B73 | Missing invoiceDueDate → 422 on POST /invoice | create_invoice workflow didn't set invoiceDueDate when neither dueDate nor invoiceDueDate was in data dict. Tripletex API requires it → 422 error, wasted write + doubled retry GETs. | ✅ FIXED — create_invoice now defaults invoiceDueDate to invoiceDate + 14 days (standard Norwegian payment terms) when not explicitly provided. Eliminates the 422. | entry-level rate from Tripletex > 0. (3) Parallelized activity+VAT lookups with `asyncio.gather`. |

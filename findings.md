...
- Final prompt-corpus sweep from docs/tasks.csv shows the heaviest families are create_supplier_invoice (7), create_invoice (6), reconcile_bank_statement (6), register_payment (4), create_customer+create_employee+create_project (4), create_customer (4), register_employment (4), and register_expense (4). The hardened canonical workflows now cover the highest-volume families directly instead of relying on fallback loops.
- Current canonical workflow mapping for the highest-volume prompt families is: supplier invoices -> create_supplier_invoice, outbound invoices -> create_invoice, statement matching -> reconcile_bank_statement, invoice settlements -> register_payment or register_fx_payment, employee onboarding -> register_employment, expense receipts -> register_expense, and project billing -> register_time + create_project_invoice or fixed-price create_project_invoice.
